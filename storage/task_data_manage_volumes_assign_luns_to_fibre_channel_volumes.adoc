---
permalink: storage/task_data_manage_volumes_assign_luns_to_fibre_channel_volumes.html 
sidebar: sidebar 
keywords: volumes, fc, luns, access group, assign 
summary: 볼륨 액세스 그룹에서 파이버 채널 볼륨의 LUN 할당을 변경할 수 있습니다.  볼륨 액세스 그룹을 생성할 때 파이버 채널 볼륨 LUN을 할당할 수도 있습니다. 
---
= LUN을 파이버 채널 볼륨에 할당
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
볼륨 액세스 그룹에서 파이버 채널 볼륨의 LUN 할당을 변경할 수 있습니다.  볼륨 액세스 그룹을 생성할 때 파이버 채널 볼륨 LUN을 할당할 수도 있습니다.

새로운 파이버 채널 LUN을 할당하는 것은 고급 기능이며 연결 호스트에 알 수 없는 결과를 초래할 수 있습니다.  예를 들어, 새 LUN ID가 호스트에서 자동으로 검색되지 않을 수 있으며, 호스트는 새 LUN ID를 검색하기 위해 다시 검색해야 할 수도 있습니다.

. *관리* > *액세스 그룹*을 선택합니다.
. 편집하려는 액세스 그룹에 대한 작업 아이콘을 클릭합니다.
. 나타나는 메뉴에서 **편집**을 선택합니다.
. *볼륨 액세스 그룹 편집* 대화 상자의 *LUN ID 할당*에서 *LUN 할당* 목록의 화살표를 클릭합니다.
. LUN을 할당하려는 목록의 각 볼륨에 대해 해당 *LUN* 필드에 새 값을 입력합니다.
. *변경 사항 저장*을 클릭하세요.

