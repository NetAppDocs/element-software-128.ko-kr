...
summary: SetNodeSSLCertificate 메서드를 사용하면 관리 노드에 대한 사용자 SSL 인증서와 개인 키를 설정할 수 있습니다. 
---
...
[role="lead"]
당신은 사용할 수 있습니다 `SetNodeSSLCertificate` 관리 노드에 대한 사용자 SSL 인증서와 개인 키를 설정하는 방법입니다.
...
자격증
 a| 
인증서의 PEM 인코딩된 텍스트 버전입니다.*참고:* 노드 또는 클러스터 인증서를 설정할 때 인증서에는 serverAuth에 대한 extendedKeyUsage 확장이 포함되어야 합니다.  이 확장 기능을 사용하면 일반적인 운영 체제와 브라우저에서 오류 없이 인증서를 사용할 수 있습니다.  확장자가 없으면 API는 인증서를 유효하지 않은 것으로 간주하여 거부합니다.
 a| 
끈
 a| 
None
