...
summary: ListSyncJobs 메서드를 사용하면 Element 스토리지 클러스터에서 실행 중인 동기화 작업에 대한 정보를 가져올 수 있습니다.  이 메서드는 슬라이스, 복제, 블록 및 원격 동기화 작업에 대한 정보를 반환합니다. 
---
= ListSyncJobs
:allow-uri-read: 
:icons: font
...
[role="lead"]
당신은 사용할 수 있습니다 `ListSyncJobs` Element 스토리지 클러스터에서 실행 중인 동기화 작업에 대한 정보를 가져오는 방법입니다.  이 메서드는 슬라이스, 복제, 블록 및 원격 동기화 작업에 대한 정보를 반환합니다.
...
동기화 작업
 a| 
현재 시스템에서 실행 중인 동기화 프로세스를 설명하는 개체 목록입니다.
 a| 
xref:reference_element_api_syncjob.adoc[동기화 작업]정렬
