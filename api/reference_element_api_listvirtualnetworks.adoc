...
summary: ListVirtualNetworks 메서드를 사용하면 클러스터에 구성된 모든 가상 네트워크를 나열할 수 있습니다. 
---
= 가상 네트워크 목록
:allow-uri-read: 
:icons: font
...
[role="lead"]
당신은 사용할 수 있습니다 `ListVirtualNetworks` 클러스터에 구성된 모든 가상 네트워크를 나열하는 방법입니다.

이 방법을 사용하면 클러스터의 가상 네트워크 설정을 확인할 수 있습니다.  이 방법에는 필수 매개변수가 없습니다.  하지만 결과를 필터링하려면 하나 이상의 virtualNetworkID 또는 virtualNetworkTag 값을 전달할 수 있습니다.
...
가상네트워크ID
 a| 
단일 가상 네트워크 목록을 필터링하기 위한 네트워크 ID입니다.
 a| 
정수
 a| 
None
...
가상네트워크태그
 a| 
단일 가상 네트워크에 대한 목록을 필터링하기 위한 네트워크 태그입니다.
 a| 
정수
 a| 
None
