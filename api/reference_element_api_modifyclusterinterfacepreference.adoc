---
permalink: api/reference_element_api_modifyclusterinterfacepreference.html 
sidebar: sidebar 
keywords: modify,modifying,edit,editing,change,changing,cluster,interface,preference,modifyclusterinterfacepreference 
summary: ModifyClusterInterfacePreference 메서드를 사용하면 Element 소프트웨어를 실행하는 스토리지 클러스터와 통합된 시스템에서 기존 클러스터 인터페이스 기본 설정을 변경할 수 있습니다.  이 방법은 내부적으로 사용됩니다. 
---
= 클러스터 인터페이스 환경 설정 수정
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
그만큼 `ModifyClusterInterfacePreference` 이 방법을 사용하면 Element 소프트웨어를 실행하는 스토리지 클러스터와 통합된 시스템에서 기존 클러스터 인터페이스 기본 설정을 변경할 수 있습니다.  이 방법은 내부적으로 사용됩니다.



== 매개변수

이 방법에는 다음과 같은 입력 매개변수가 있습니다.

|===


| 이름 | 설명 | 유형 | 기본값 | 필수의 


 a| 
이름
 a| 
수정할 클러스터 인터페이스 기본 설정의 이름입니다.
 a| 
끈
 a| 
None
 a| 
예



 a| 
값
 a| 
클러스터 인터페이스 기본 설정의 새로운 값입니다.
 a| 
끈
 a| 
None
 a| 
예

|===


== 반환 값

이 메서드에는 반환 값이 없습니다.



== 요청 예시

이 방법에 대한 요청은 다음 예와 유사합니다.

[listing]
----
{
	"method": "ModifyClusterInterfacePreference",
	"params": {
	"name": "testname",
	"value": "newvalue"
},
	"id": 1
}
----


== 응답 예시

이 메서드는 다음 예와 유사한 응답을 반환합니다.

[listing]
----
{
    "id": 1,
    "result": {}
}
----


== 버전 이후 새로운

11.0
