...
summary: Fibre Channel 노드의 GetNodeHardwareInfo 메서드는 다음 예와 유사한 응답을 반환합니다. 
---
= GetNodeHardwareInfo(Fibre Channel 노드에 대한 출력)
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
그만큼 `GetNodeHardwareInfo` Fibre Channel 노드의 경우 이 메서드는 다음 예와 유사한 응답을 반환합니다.
