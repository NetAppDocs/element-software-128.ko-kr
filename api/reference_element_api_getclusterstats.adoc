...
summary: GetClusterStats 메서드를 사용하면 클러스터에 대한 상위 수준의 활동 측정값을 검색할 수 있습니다.  반환된 값은 클러스터 생성 이후 누적됩니다. 
---
= 클러스터 통계 가져오기
:allow-uri-read: 
:icons: font
...
[role="lead"]
당신은 사용할 수 있습니다 `GetClusterStats` 클러스터에 대한 고수준 활동 측정값을 검색하는 방법입니다.  반환된 값은 클러스터 생성 이후 누적됩니다.
