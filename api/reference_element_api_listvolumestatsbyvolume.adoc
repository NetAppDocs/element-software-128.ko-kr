...
summary: ListVolumeStatsByVolume 메서드를 사용하면 볼륨별로 모든 볼륨에 대한 높은 수준의 활동 측정값을 나열할 수 있습니다.  볼륨 생성부터 값이 누적됩니다. 
---
= 볼륨별 통계 목록
:allow-uri-read: 
:icons: font
...
[role="lead"]
당신은 사용할 수 있습니다 `ListVolumeStatsByVolume` 각 볼륨별로 높은 수준의 활동 측정값을 나열하는 방법입니다.  볼륨 생성부터 값이 누적됩니다.
