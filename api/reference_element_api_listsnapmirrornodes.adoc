...
[role="lead"]
Element 소프트웨어 웹 UI는 다음을 사용합니다. `ListSnapMirrorNodes` 원격 ONTAP 클러스터의 노드 목록을 가져오는 방법입니다.
...
스냅미러엔드포인트ID
 a| 
제공된 경우 시스템은 지정된 snapMirrorEndpointID가 있는 엔드포인트의 노드를 나열합니다.  제공되지 않으면 시스템은 알려진 모든 SnapMirror 엔드포인트의 노드를 나열합니다.
 a| 
정수
 a| 
None
...
ONTAP 클러스터의 노드 목록입니다.
 a| 
xref:reference_element_api_snapmirrornode.adoc[스냅미러노드]정렬
