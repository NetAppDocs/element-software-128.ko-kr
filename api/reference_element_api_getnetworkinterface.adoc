...
summary: GetNetworkInterface 메서드를 사용하면 노드의 네트워크 인터페이스에 대한 정보를 얻을 수 있습니다. 
---
= GetNetworkInterface
:allow-uri-read: 
:icons: font
...
[role="lead"]
당신은 사용할 수 있습니다 `GetNetworkInterface` 노드의 네트워크 인터페이스에 대한 정보를 얻는 방법입니다.
...
인터페이스
 a| 
각 노드에 대한 정보를 얻을 인터페이스의 이름입니다. 가능한 값:
...
힘
 a| 
클러스터의 모든 노드에서 실행하려면 이 매개변수를 true로 설정합니다.
 a| 
부울
 a| 
거짓
...
노드
 a| 
스토리지 클러스터의 각 스토리지 노드에 대한 인터페이스를 설명하는 객체 배열입니다.  배열 내의 각 객체에는 다음 항목이 포함되어 있습니다.

* nodeID: (정수) 인터페이스 정보가 적용되는 스토리지 클러스터의 스토리지 노드 ID입니다.
