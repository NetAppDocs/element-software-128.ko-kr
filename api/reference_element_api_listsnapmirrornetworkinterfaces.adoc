...
[role="lead"]
Element 소프트웨어 웹 UI는 다음을 사용합니다. `ListSnapMirrorNetworkInterfaces` 원격 ONTAP 시스템에서 사용 가능한 모든 SnapMirror 인터페이스를 나열하는 방법입니다.
...
스냅미러엔드포인트ID
 a| 
지정된 엔드포인트 ID와 연결된 네트워크 인터페이스만 반환합니다.  엔드포인트 ID가 제공되지 않으면 시스템은 알려진 모든 SnapMirror 엔드포인트의 인터페이스를 나열합니다.
 a| 
정수
 a| 
None
...
snapMirrorNetworkInterfaces
 a| 
원격 ONTAP 스토리지 시스템에서 사용 가능한 SnapMirror 네트워크 인터페이스 목록입니다.
 a| 
xref:reference_element_api_snapmirrornetworkinterface.adoc[스냅미러네트워크인터페이스]정렬
