...
summary: GetVolumeStats 메서드를 사용하면 단일 볼륨에 대한 높은 수준의 활동 측정값을 얻을 수 있습니다.  볼륨이 생성된 이후부터 값이 누적됩니다. 
---
= 볼륨 통계 가져오기
:allow-uri-read: 
:icons: font
...
[role="lead"]
당신은 사용할 수 있습니다 `GetVolumeStats` 단일 볼륨에 대한 높은 수준의 활동 측정을 얻는 방법입니다.  볼륨이 생성된 이후부터 값이 누적됩니다.
